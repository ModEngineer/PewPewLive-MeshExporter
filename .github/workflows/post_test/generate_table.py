...
sortedJsons = sorted(jsons, reverse=True)
table = "| Test |"
tablerow2 = "\n| --- |"
tracebacktable = ""
for version in sortedJsons:
    cleanVersionString = version.split("v")[-1]
    table+=f" {cleanVersionString} |"
    tablerow2+=" --- |"
    tracebacktable+='Tracebacks for Blender version ' + cleanVersionString + ':\n```py\n' + ("\n\n--------------------\n\n".join([tb for tb in sortedJsons[version]["tracebacks"] if type(tb)==str])) + '```\n'
...
for index, test in enumerate(["Add-on registration", "Vertex color operator", "Object export with local coordinates, only selected objects", "Object export without local coordinates, all objects", "Add-on unregistration", "Tests (this will show as an error if the test sotware had an error)"]):
    table+=f"\n| {test} |"
    for version in sortedJsons:
        table+=f" {resultcharacters[int(bool(sortedJsons[version]['exitcode'] & (1<<index)))]} |"
