...
sys.path.append(
    os.path.join(os.environ["GITHUB_WORKSPACE"], "tempmodulefolder"))

test_results = {"exitcode": 0, "tracebacks": []}


def save_test_results(results):
    with open(os.path.join(os.environ["GITHUB_WORKSPACE"], ".github",
                           "workflows", "blender_tests", "test_results.json"),
              "wt",
...
try:
    ...
    if bpy.app.version >= (2, 90, 0):
        bpy.ops.mesh.primitive_cube_add(location=(0, 0, 0),
                                        rotation=(0, 0, 0),
                                        scale=(1, 1, 1))
        bpy.ops.mesh.primitive_cube_add(location=(5.25, 5.25, 5.25),
                                        rotation=(56, 241, 72),
                                        scale=(1, 1, 1))
    else:
    ...
    if bpy.context.scene.objects[0].location == mathutils.Vector(
        (5.25, 5.25, 5.25)):
        currentobj = bpy.context.scene.objects[0]
    elif bpy.context.scene.objects[1].location == mathutils.Vector(
        (5.25, 5.25, 5.25)):
    ...
    for edge in bm.edges:
        if [round(coord)
                for coord in list(edge.verts[0].co)] in seamVertexCoords and [
                    round(coord) for coord in list(edge.verts[1].co)
                ] in seamVertexCoords:
            edge.seam = True
    ...
    if bpy.app.version > (2, 79, 0):
        ...
        currentobj.select = True
    try:
        bpy.ops.pewpew_live_meshexporter.exportmeshfromscene(
            filepath=os.path.join(os.environ["GITHUB_WORKSPACE"], ".github",
                                  "workflows", "blender_tests", "test.lua"),
            max_decimal_digits=3,
            multiplier=5,
            only_selected=True,
            use_local=True,
            exclude_seamed_edges=True,
            export_color=True,
            color_decompressor_location="decompresscolors",
            use_segments=False,
            use_fixedpoint=False,
            apply_modifiers=False,
            as_animation="DISABLED")
        test_results["tracebacks"].append(None)
    except Exception:
        test_results["tracebacks"].append(traceback.format_exc())
    ...
    try:
        bpy.ops.pewpew_live_meshexporter.exportmeshfromscene(
            filepath=os.path.join(os.environ["GITHUB_WORKSPACE"], ".github",
                                  "workflows", "blender_tests", "test2.lua"),
            max_decimal_digits=3,
            multiplier=5,
            only_selected=False,
            use_local=False,
